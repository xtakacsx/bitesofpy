...
def _get_runtime():
    return max([movie.get("runtime") for movie in get_tree().iter('movie')])
...
def get_movie_longest_runtime():
    ...
    return [movie.get("title") for movie in get_tree().iter('movie') if movie.get("runtime") == _get_runtime()][0]
