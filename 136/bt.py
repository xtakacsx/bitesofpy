    ...
    def get_val(arg):
        try:
            if isinstance(arg, int):
                return Bloodtype(arg).value
            elif isinstance(arg, str):
                return blood_type_text[arg].value
            elif isinstance(arg, Bloodtype):
                return arg.value
        except Exception:
            raise ValueError

    try:
        assert isinstance(donor, (int, str, Bloodtype)) or isinstance(recipient, (int, str, Bloodtype))
    except AssertionError:
        raise TypeError

    donor_val = get_val(donor)
    recipient_val = get_val(recipient)

    if len([x for x in _particular_antigen_comp(donor_val, recipient_val) if x < 0]) > 0:
        return False
    else:
        return True
